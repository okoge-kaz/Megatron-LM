...
#PBS -q rt_HF
#PBS -N llama-3.3-70b
#PBS -l select=16:ncpus=192:ngpus=8
#PBS -l walltime=144:00:00
#PBS -j oe
#PBS -koed
#PBS -V
#PBS -o outputs/Llama-3.3-70B/
#PBS -P gcg51558

set -e

cd $PBS_O_WORKDIR
mkdir -p outputs/Llama-3.3-70B
...
CHECKPOINT_DIR=/groups/gag51395/checkpoints/hf-to-megatron/Llama-3.3-70b-Instruct/tp${TENSOR_PARALLEL_SIZE}-pp${PIPELINE_PARALLEL_SIZE}
CHECKPOINT_SAVE_DIR=/groups/gag51395/checkpoints/Llama-3.3-70b-Instruct/tp${TENSOR_PARALLEL_SIZE}-pp${PIPELINE_PARALLEL_SIZE}-ct${CONTEXT_PARALLEL_SIZE}/LR${LR}-MINLR${MIN_LR}-WD${WEIGHT_DECAY}
...
# job name
JOB_NAME="Llama-3.3-70b-ABCI-3.0-${NODE_TYPE}-${NUM_NODES}node-${NUM_GPUS}gpu-${SEQ_LENGTH}s-DP=${DATA_PARALLEL_SIZE}-TP=${TENSOR_PARALLEL_SIZE}-PP=${PIPELINE_PARALLEL_SIZE}-BS=${GLOBAL_BATCH_SIZE}-LR=${LR}-MINLR=${MIN_LR}-WARMUP=${LR_WARMUP_STEPS}-WD=${WEIGHT_DECAY}-GC=${GRAD_CLIP}"
