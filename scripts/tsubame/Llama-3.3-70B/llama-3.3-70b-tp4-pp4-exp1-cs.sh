...
#$ -l node_f=32
#$ -l h_rt=02:23:10:00
#$ -o outputs/Llama-3.3-70B/$JOB_ID.log
#$ -e outputs/Llama-3.3-70B/$JOB_ID.log
#$ -p -3
...
PIPELINE_MODEL_CHUNK=1
LAYERS_PER_VIRTUAL_PIPELINE_STAGE=$((${NUM_LAYERS} / ${PIPELINE_PARALLEL_SIZE} / ${PIPELINE_MODEL_CHUNK}))
...
CHECKPOINT_DIR=/gs/bs/tga-NII-LLM/checkpoints/hf-to-megatron/Llama-3.3-70b-Instruct/tp${TENSOR_PARALLEL_SIZE}-pp${PIPELINE_PARALLEL_SIZE}-te-v1.6
CHECKPOINT_SAVE_DIR=/gs/bs/tga-NII-LLM/checkpoints/Llama-3.3-70B-Instruct/tp${TENSOR_PARALLEL_SIZE}-pp${PIPELINE_PARALLEL_SIZE}-ct${CONTEXT_PARALLEL_SIZE}/exp1-current-scaling/LR${LR}-MINLR${MIN_LR}-WD${WEIGHT_DECAY}
...
# job name
JOB_NAME="Llama-3.3-70b-CurrentScaling-TSUBAME-${NODE_TYPE}-${NUM_NODES}node-${NUM_GPUS}gpu-${SEQ_LENGTH}s-DP=${DATA_PARALLEL_SIZE}-TP=${TENSOR_PARALLEL_SIZE}-PP=${PIPELINE_PARALLEL_SIZE}-BS=${GLOBAL_BATCH_SIZE}-LR=${LR}-MINLR=${MIN_LR}-WARMUP=${LR_WARMUP_STEPS}-WD=${WEIGHT_DECAY}-GC=${GRAD_CLIP}"
...
if [[ ${PIPELINE_MODEL_CHUNK} -gt 1 ]]; then
...
mpirun -np $NUM_GPUS \
  ...
  -x LD_LIBRARY_PATH \
  -x PATH \
  -x NVTE_FPROP_CURRENT_SCALING=1 \
  -x NVTE_DGRAD_CURRENT_SCALING=1 \
  -x NVTE_WGRAD_CURRENT_SCALING=1 \
  -x NVTE_WGRAD_INPUT_CURRENT_SCALING=1 \
  -x NVTE_DGRAD_WEIGHT_CURRENT_SCALING=1 \
  -x NVTE_FP8_CURRENT_SCALING_FORMAT="HYBRID" \
  -x NVTE_2X_ACCUMULATION_FPROP=1 \
  -x NVTE_2X_ACCUMULATION_DGRAD=1 \
  -x NVTE_2X_ACCUMULATION_WGRAD=1 \
  -bind-to none \
